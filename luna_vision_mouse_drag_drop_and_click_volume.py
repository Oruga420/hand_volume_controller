...
screenW, screenH = pyautogui.size()  # Define screen width and height
class HandDetector():
...
# Variables to keep track of the drag, click, and volume control states
dragging = False
clicking = False
vol_control = False
...
while True:
    ...
    if len(lmList) != 0:
        ...
        # Check for thumbs-up gesture for clicking
        if lmList[4][2] < lmList[3][2] and lmList[4][2] < lmList[2][2]:  # Thumb tip is above the lower joints
            if not clicking:
                pyautogui.click()
                clicking = True
        else:
        ...
        # Volume control with all five fingers
        fingers = detector.fingersUp()
        if fingers.count(1) == 5:  # All fingers are up
            vol_control = True
            pinky_tip_y = lmList[20][2]
            index_tip_y = lmList[8][2]
            vol_range_height = hCam // 3  # Set the range for volume control to be one third of the camera height
            vol_level = np.interp(pinky_tip_y - index_tip_y, [0, vol_range_height], [0, 100])
